// GigiCopyNMHost - Native Messaging host for Chrome
// Reads the menubar helper's queue.json and prints clips to stdout using
// Chrome Native Messaging framing (4-byte little-endian length + UTF-8 JSON).
//
// MVP behavior:
// - Waits for a single incoming message (e.g., {"type":"drain"})
// - Loads queued clips, emits each as {type:"clip", ...}
// - Truncates the queue file to an empty array and exits
//
// Adjust `helperBundleId` if you changed your menubar app's bundle identifier.
...
// MARK: - Paths
let helperBundleId = "Flexipie.GigiCopyHelper" // update if needed
...
func readNMMessage() -> [String: Any]? {
    ...
    guard let lenData = stdinHandle.read(upToCount: 4), lenData.count == 4 else {
        return nil
    }
    let length = lenData.withUnsafeBytes { ptr -> UInt32 in
        // NM specifies 4-byte little-endian length prefix
        return ptr.load(as: UInt32.self).littleEndian
    }
    let toRead = Int(length)
    guard toRead > 0, let payload = stdinHandle.read(upToCount: toRead), payload.count == toRead else {
        return nil
    }
...
// Entry point
// Wait for a message (e.g., {"type":"drain"}), then drain and exit.
_ = readNMMessage()
